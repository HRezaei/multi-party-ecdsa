...
pub struct KeyGenStage2Result {
    pub vss_scheme_s: VerifiableSS<Secp256k1>,
    pub secret_shares_s: Vec<Scalar<Secp256k1>>,
    pub index_s: usize,
}
//
...
pub struct KeyGenStage3Input {
    ...
    pub secret_shares_vec_s: Vec<Scalar<Secp256k1>>,
    pub y_vec_s: Vec<Point<Secp256k1>>,
    pub params_s: Parameters,
    pub index_s: usize,
}
...
pub struct KeyGenStage3Result {
    ...
    pub dlog_proof_s: DLogProof<Secp256k1, Sha256>,
}
//
// As per page 13 on https://eprint.iacr.org/2020/540.pdf:
...
pub struct KeyGenStage4Input {
    pub params_s: Parameters,
    pub dlog_proof_vec_s: Vec<DLogProof<Secp256k1, Sha256>>,
    pub y_vec_s: Vec<Point<Secp256k1>>,
}
...
pub struct SignStage2Result {
    pub gamma_i_vec: Vec<(MessageB, Scalar<Secp256k1>)>,
    pub w_i_vec: Vec<(MessageB, Scalar<Secp256k1>)>,
}
...
pub fn sign_stage2(input: &SignStage2Input) -> Result<SignStage2Result, ErrorType> {
    ...
    for j in 0..input.l_ttag - 1 {
        ...
        // See gg20_sign_offline_stages for a demo of how this value is encrypted using a key shared
...
pub struct SignStage3Result {
    pub alpha_vec_gamma: Vec<Scalar<Secp256k1>>,
    pub alpha_vec_w: Vec<Scalar<Secp256k1>>,
}
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct SignStage3Input {
    pub dk_s: DecryptionKey,
    pub k_i_s: Scalar<Secp256k1>,
    pub m_b_gamma_s: Vec<MessageB>,
    pub m_b_w_s: Vec<MessageB>,
    pub g_w_i_s: Vec<Point<Secp256k1>>,
    pub index_s: usize,
    pub ttag_s: usize,
}
...
pub struct SignStage4Input {
    pub alpha_vec_s: Vec<Scalar<Secp256k1>>,
    pub beta_vec_s: Vec<Scalar<Secp256k1>>,
    pub miu_vec_s: Vec<Scalar<Secp256k1>>,
    pub ni_vec_s: Vec<Scalar<Secp256k1>>,
...
pub fn sign_stage5(input: &SignStage5Input) -> Result<SignStage5Result, ErrorType> {
    ...
    let Rdash_vec_i = Rvec_i.clone() * input.sign_keys.k_i.clone();
    Ok(SignStage5Result {
        R: Rvec_i,
        R_dash: Rdash_vec_i,
    })
...
pub fn sign_stage7(input: &SignStage7Input) -> Result<SignStage7Result, ErrorType> {
    let s_vec: Vec<Scalar<Secp256k1>> = input.local_sig_vec.iter().map(|a| a.s_i.clone()).collect();
...
pub fn check_sig(r: &Scalar<Secp256k1>, s: &Scalar<Secp256k1>, msg: &BigInt, pk: &Point<Secp256k1>) {
    ...
    let slice = pk.to_bytes(false).to_vec();
...
mod tests {
    ...
    pub fn keygen_orchestrator(params: Parameters) -> Result<KeyPairResult, ErrorType> {
        ...
        let y_vec = (0..params.share_count)
            .map(|i| decom_vec_l[i as usize].y_i.clone())
        ...
        let pk_vec_l = (0..params.share_count)
            .map(|i| dlog_proof_vec_l[i as usize].pk.clone())
            .collect::<Vec<Point<Secp256k1>>>();
        ...
        let xi_vec = (0..=params.threshold)
            .map(|i| shared_keys_vec_l[i as usize].x_i.clone())
            .collect::<Vec<Scalar<Secp256k1>>>();
        let vss_scheme_for_test = vss_scheme_vec_l.clone();
        let index_vector: Vec<u16> = (0..=params.threshold).into_iter().map(|x1| {x1 as u16}).collect::<Vec<u16>>();
        let x = vss_scheme_for_test[0]
            .clone()
            .reconstruct(&index_vector, &xi_vec);
        let sum_u_i = party_keys_vec_l
            .iter()
            .fold(Scalar::<Secp256k1>::zero(), |acc, x| acc + x.u_i.clone());
        assert_eq!(x, sum_u_i);
        // test code ends.
    ...
    /*pub fn orchestrate_sign(
        ...
        let gamma_i_vec = (0..ttag)
            ...
            .collect::<Vec<Scalar<Secp256k1>>>();
        let w_i_vec = (0..ttag).map(|i| sign_keys_vec[i].w_i).collect::<Vec<Scalar<Secp256k1>>>();
        ...
        let g_wi_vec: Vec<Point<Secp256k1>> = (0..ttag).map(|a| sign_keys_vec[a].g_w_i).collect();
        for i in 0..ttag {
        ...
        let delta_vec: Vec<Scalar<Secp256k1>> = res_stage4_vec.iter().map(|val| val.delta_i).collect();
        ...
        let R_vec: Vec<Point<Secp256k1>> = result_stage5_vec.iter().map(|a| a.R).collect();
        let R_dash_vec: Vec<Point<Secp256k1>> = result_stage5_vec.iter().map(|a| a.R_dash).collect();

        let message_bn_l = Sha256::new()
            .chain_bigint(&BigInt::from_bytes(bytes_to_sign))
            .result_bigint();
        // sigma_vec This is just to facilitate writing the code. It should never be collected like
        // this IRL.
        let sigma_vec: Vec<Scalar<Secp256k1>> = res_stage4_vec.iter().map(|val| val.sigma_i).collect();
