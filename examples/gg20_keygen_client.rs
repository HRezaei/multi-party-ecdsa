...
use multi_party_ecdsa::protocols::multi_party_ecdsa::gg_2020::orchestrate::{
    ...
    KeyGenStage1Result, KeyGenStage2Input, KeyGenStage3Input, KeyGenStage4Input
};
...
fn main() {
    ...
    for i in 1..=params.share_count {
        point_vec.push(decom1_vec[(i - 1) as usize].y_i.clone());
        if i != party_num_int {
            let key_bn: BigInt = (decom1_vec[(i - 1) as usize].y_i.clone()
                * res_stage1.party_keys_l.u_i.clone())
                .x_coord()
                .unwrap();
    ...
    for (k, i) in (1..=params.share_count).enumerate() {
        if i != party_num_int {
            ...
            let plaintext = BigInt::to_bytes(&res_stage2.secret_shares_s[k].to_bigint());
    ...
    for i in 1..=params.share_count {
        if i == party_num_int {
            party_shares.push(res_stage2.secret_shares_s[(i - 1) as usize].clone());
        } else {
